...
/**
 * 📌 HERO — COMPONENTE DE DISEÑO
 * 
 * Este componente SOLO se encarga de la interfaz de usuario (UI).
 * Toda la lógica (imágenes, carga, subida) está en el hook useHero.
 * 
 * ✅ Buenas prácticas:
 * - Separación de lógica y diseño (SoC)
 * - Componente presentacional (solo UI)
 * - Usa useHero para toda la lógica
 * - Toasts centralizados con el helper
 * - Carrusel reutilizable ImageCarousel
 */
...
export default function Hero() {
  // =============================================
  // 🧠 LÓGICA (extraída a useHero)
  // =============================================
  const { images, isUploading, isAdmin, uploadImage, removeImage } = useHero();
      ...
      <div className="relative max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 items-center w-full">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2 justify-center lg:justify-start pt-1">
            ...
            <a href={empresaConfig.facebook} target="_blank" rel="noopener noreferrer" style={{ backgroundColor: socialColors.facebook }} className="text-white p-2.5 rounded-full transition-all hover:scale-110 shadow-md hover:shadow-lg" aria-label="Facebook"><FaFacebook size={18} /></a>
            <a href={empresaConfig.instagram} target="_blank" rel="noopener noreferrer" style={{ backgroundColor: socialColors.instagram }} className="text-white p-2.5 rounded-full transition-all hover:scale-110 shadow-md hover:shadow-lg" aria-label="Instagram"><FaInstagram size={18} /></a>
            <a href={empresaConfig.tiktok} target="_blank" rel="noopener noreferrer" style={{ backgroundColor: socialColors.tiktok }} className="text-white p-2.5 rounded-full transition-all hover:scale-110 shadow-md hover:shadow-lg" aria-label="TikTok"><FaTiktok size={18} /></a>
            <a href={`https://wa.me/${empresaConfig.whatsapp}`} target="_blank" rel="noopener noreferrer" style={{ backgroundColor: socialColors.whatsapp }} className="text-white p-2.5 rounded-full transition-all hover:scale-110 shadow-md hover:shadow-lg" aria-label="WhatsApp"><FaWhatsapp size={18} /></a>
          </div>
          ...
          <div className="w-full max-w-md bg-white/90 backdrop-blur-md p-6 rounded-3xl border-2 border-[#00D2D3] shadow-2xl hover:shadow-[#00D2D3]/30 transition-all duration-300">
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="flex items-center gap-3 text-sm bg-[#303030]/5 p-2.5 rounded-xl hover:bg-[#303030]/10 transition">
                ...
                <a href={`https://wa.me/${empresaConfig.whatsapp}`} target="_blank" rel="noopener noreferrer" className="font-medium text-[#00D2D3] hover:text-[#603060] transition-colors text-sm">
                  {empresaConfig.telefono}
                </a>
              </div>
              <div className="flex items-center gap-3 text-sm bg-[#303030]/5 p-2.5 rounded-xl hover:bg-[#303030]/10 transition">
                <SiGmail className="text-[#EA4335] text-lg" />
                <a href={`mailto:${empresaConfig.email}`} className="font-medium text-[#603060] hover:text-[#7D5FFF] transition-colors text-xs sm:text-sm truncate">
                  {empresaConfig.email}
                </a>
              </div>
            </div>

            <a href={`https://wa.me/${empresaConfig.whatsapp}?text=Hola%20${empresaConfig.nombre}%2C%20quiero%20consultar%20sobre...`} target="_blank" rel="noopener noreferrer" className="mt-4 w-full flex items-center justify-center gap-2 bg-gradient-to-r from-[#00D2D3] to-[#7D5FFF] hover:from-[#7D5FFF] hover:to-[#00D2D3] text-white font-bold py-3 rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-105 text-sm">
              <FaWhatsapp size={18} />
